...
var JobQueue = module.exports = Class( Object, function($proto) {
   ...
   * @param {Number} [options.timeout=0] Maximum number of seconds to block, 0 means block indefinitely. The value must be an integer.
   ...
   *
   * @param {Object} options
   * @param {String} options.type     Omit type to handle all the job types
   * @param {Number} options.timeout  Maximum number of seconds to block, omit to block indefinitely.
   ...
   * @api public
   */
  $proto.process = function(options, cb) {
    ...
    function runJob() {
      self.dequeue({type:options.type, state:STATES.RUN, timeout:options.timeout}, function(err, jobId){
  ...
  /**
   * Disconnect the connection to Redis server forcibly
   */
  $proto.end = function() {
    this._rc.end();
    return this;
